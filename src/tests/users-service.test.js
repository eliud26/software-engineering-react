...
describe('createUser', () => {
    const ripley = {    // sample user to insert
        username: 'ellenripley',
        password: 'lv426',
        email: 'ellenripley@aliens.com'
    };

    beforeAll(() => {   // setup test
        return deleteUsersByUsername(ripley.username); // remove what we inserted
    });

    afterAll(() => {    // clean up test
        return deleteUsersByUsername(ripley.username); // remove what we inserted
    });

    test('can insert with REST API', async () => {
        const newUser = await createUser(ripley);   // insert new user
        console.log(newUser._id);
        expect(newUser.username)        // verify inserted properties
            .toEqual(ripley.username);  // match data sent
